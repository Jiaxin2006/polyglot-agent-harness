...
- **PR 绑定测试**：若该用例将随 PR 交付，必须满足 `pr-workflow` 的 E2E 要求（用例与功能同 PR；Test plan 含 E2E/测试命令 + 期望输出，**不写** `cargo fmt`）
- **Rust host 代码**：交付前须通过 `cargo fmt --all -- --check`（失败则 `cargo fmt --all` 后 re-check，与功能变更同 commit）— **本地门禁**，不写入 PR 草稿 Test plan
